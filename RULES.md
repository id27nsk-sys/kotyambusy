...
- **HAPTIC_ATTACK_STRICT**: (ДЕАКТИВИРОВАНО) Вибрация заменена на SCREEN_SHAKE_STRICT для унификации опыта.
- **HERO_SCALE_STRICT**: Размер круга героя увеличен (280px Standard / 200px Micro).
- **SCREEN_SHAKE_STRICT**: Резкая тряска экрана (body) при Кусе — основной стандарт отдачи для всех ОС.
- **IMPACT_DYNAMICS_STRICT**: Эффект «Куся» должен быть молниеносным (500ms) и визуально экстремальным.
- **SOFT_TRANSITION_STRICT**: Любая плановая смена фото героя (цикл поглаживаний или выбор героя) должна сопровождаться мягким визуальным переходом (fade-эффект) длительностью 0.3с.
