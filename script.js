window.onload = function() {
    ...
    const achList = [
        { id: 'first_10', n: 'Первые шаги 🐾', cond: (d) => Object.values(d.stats).some(v => v >= 10) },
        { id: 'collector', n: 'Фотограф 📸', cond: (d) => (d.album.basya.length + d.album.savely.length) >= 3 },
        { id: 'rich', n: 'Богатей 💰', cond: (d) => d.coins >= 100 }
    ];
    ...
    function updateAchievements() {
        achList.forEach(ach => {
            ...
            if (data.unlockedAchs.includes(ach.id) && !document.getElementById(`ach-${ach.id}`)) {
                ...
                div.className = 'achievement'; div.id = `ach-${ach.id}`;
                div.innerText = ach.n;
    ...
    // Универсальный обработчик клика по коту (мышь + палец)
    el.sec.onpointerdown = (e) => {
        // Не блокируем (e.preventDefault), чтобы кнопки работали
        if (activeCat === 'custom' && !data.url) { el.fileI.click(); return; }
        
        data.stats[activeCat]++; data.coins++;
    ...
    el.fileI.onchange = (e) => {
        ...
        reader.onload = (ev) => {
            data.url = ev.target.result; activeCat = 'custom';
            save(); update(true);
        };
